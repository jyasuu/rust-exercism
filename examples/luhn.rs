...
pub fn is_valid(code: &str) -> bool {
    ...
    // Iterate over the characters in reverse, skipping spaces
    for c in code.chars().rev() {
        ...
        if let Some(digit) = c.to_digit(10) {
            ...
            // If any character is not a digit or whitespace, it's invalid
            return false;
        }
    }

    // The number is valid if the sum is divisible by 10 and the code has more than one digit
    sum % 10 == 0 && code.chars().filter(|c| !c.is_whitespace()).count() > 1
}
